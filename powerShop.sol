...
contract PowerToken is ERC20, Ownable, ERC20Burnable {
    address public operator;

    event OperatorSet(address indexed operator);

    constructor() ERC20("PowerHASH", "HASH") {}
...
contract powerShop is Ownable {
    PowerToken public token;
    uint256 public priceIncreasePerDay = 30; // 0.3% = 30/10000
    ...
    struct SwapTokenInfo {
        ...
        bool isSupported;
        uint256 exchangeRate; // 汇率
        uint256 totalPower;
    }

    mapping(address => mapping(address => uint256)) public swapTokenAmount; // 用户用来兑换的代币数量
    mapping(address => SwapTokenInfo) public supportedSwapToken; // 支持的兑换代币
    ...
    event SwapTokenExchangeRateSet(
        address indexed payToken,
        uint256 exchangeRate
    );

    event PriceUpdated(address indexed payToken, uint256 powerPerPrice);

    event ReceiverAddressSet(address indexed receiver);

    event PayTokenWithdrawn(
        address indexed payToken,
    ...
    function _setSwapToken(
        ...
        require(
            supportedSwapToken[payToken].totalPower == 0,
            "Token already set"
        );
    ...
    function setSwapTokenStatus(
        address payToken,
        bool isSupported
    ) external onlyOwner {
        require(
            supportedSwapToken[payToken].tokenDecimals > 0,
            "Token not exists"
        );
        supportedSwapToken[payToken].isSupported = isSupported;
    }

    function setSwapTokenExchangeRate(
        address payToken,
        uint256 exchangeRate
    ) external onlyOwner {
        require(
        ...
        supportedSwapToken[payToken].exchangeRate = exchangeRate;

        emit SwapTokenExchangeRateSet(payToken, exchangeRate);
    }
    ...
    // 根據USDT數量返回可購買到的算力數量
    function getPowerAmountByUsdt(
        uint256 payAmount,
        address payToken
    ) public view returns (uint256) {
        // usdtAmount單位為6位小數，powerPerUsdt為18位小數
        return
            (payAmount *
                supportedSwapToken[payToken].powerPerPrice *
    ...
    function deposit(uint256 payAmount, address payToken) public {
        require(payAmount > 0, "Send USDT to buy tokens");
        SwapTokenInfo storage swapTokenInfo = supportedSwapToken[payToken];
        require(swapTokenInfo.isSupported, "Token not supported");
        ...
        swapTokenInfo.totalPower += tokensToBuy;
        swapTokenAmount[msg.sender][payToken] += tokensToBuy;
        token.mint(msg.sender, tokensToBuy);
        emit CoffeePurchased(msg.sender, payToken, payAmount, tokensToBuy);
    }

    function withdrawPower(uint256 powerAmount, address payToken) public {
        require(powerAmount > 0, "0");
        require(
            powerAmount <= swapTokenAmount[msg.sender][payToken],
            "Not enough power amount"
        );
        ...
        swapTokenInfo.totalPower -= powerAmount;
        TransferHelper.safeTransfer(payToken, msg.sender, payRefund);

        emit CoffeeWithdrawn(msg.sender, payToken, payRefund, powerAmount);
    }
    ...
    function withdrawPayToken(address payToken) external onlyOwner {
        uint256 balance = IERC20(payToken).balanceOf(address(this));
        SwapTokenInfo memory swapTokenInfo = supportedSwapToken[payToken];
        uint256 totalRefundBalance = (((swapTokenInfo.totalPower *
            swapTokenInfo.tokenDecimals) / swapTokenInfo.powerPerPrice) * 997) /
            1000;

        if (balance > totalRefundBalance) {
            uint256 withdrawAmount = balance - totalRefundBalance;
            TransferHelper.safeTransfer(
                payToken,
                tokenReceiver,
                withdrawAmount
            );
            emit PayTokenWithdrawn(payToken, tokenReceiver, withdrawAmount);
        }
    }
}
